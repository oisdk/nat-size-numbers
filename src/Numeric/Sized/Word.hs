...
instance KnownNat n =>
    ...
    complement =
        trunc . (coerce :: (Natural -> Natural) -> Word n -> Word n) complement
    shift =
        trunc .:
        (coerce :: (Natural -> Prelude.Int -> Natural) -> Word n -> Prelude.Int -> Word n)
            shift
    rotate =
        trunc .:
        (coerce :: (Natural -> Prelude.Int -> Natural) -> Word n -> Prelude.Int -> Word n)
    ...
    isSigned _ = False
    testBit =
        (coerce :: (Natural -> Prelude.Int -> Bool) -> Word n -> Prelude.Int -> Bool)
            testBit
    popCount =
        (coerce :: (Natural -> Prelude.Int) -> Word n -> Prelude.Int) popCount
