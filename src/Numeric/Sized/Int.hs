...
instance KnownNat n => Bits (Int n) where
 (.&.)        = (coerce :: CoerceBinary Integer (Int n)) (.&.)
 (.|.)        = (coerce :: CoerceBinary Integer (Int n)) (.|.)
 xor          = trunc .: (coerce :: CoerceBinary Integer (Int n)) xor
 complement   = trunc .  (coerce :: (Integer -> Integer)
                                 ->  Int n   -> Int n
                         ) complement
 shift        = trunc .: (coerce :: (Integer -> Prelude.Int -> Integer)
                                 -> Int n -> Prelude.Int -> Int n
                         ) shift
 rotate       = trunc .: (coerce :: (Integer -> Prelude.Int -> Integer)
                                 ->  Int n   -> Prelude.Int -> Int n
                         ) rotate
 bit          = trunc . Int . bit
 ...
 isSigned _   = True
 testBit      = (coerce :: (Integer -> Prelude.Int -> Bool)
                        ->  Int n   -> Prelude.Int -> Bool
                ) testBit
 popCount     = (coerce :: (Integer -> Prelude.Int)
                        ->  Int n   -> Prelude.Int
                ) popCount

trunc :: KnownNat n => Int n -> Int n
trunc x | testBit x (fromInteger (natVal x) - 1) = x .|. minBound
        | otherwise = x .&. maxBound

convBinary :: KnownNat n => CoerceBinary Integer (Int n)
convBinary f = trunc .: coerce f

instance KnownNat n => Num (Int n) where
  (+) = convBinary (+)
  (*) = convBinary (*)
...
instance KnownNat n => Eq (Int n) where
  (==) = (==) `on` getInt . trunc

instance KnownNat n => Ord (Int n) where
  compare = compare `on` getInt . trunc

instance KnownNat n => Real (Int n) where
  toRational = toRational . getInt

instance KnownNat n => Enum (Int n) where
  fromEnum = fromEnum . getInt
  toEnum = trunc . Int . toEnum
  enumFrom x = [x..maxBound]

instance KnownNat n => Integral (Int n) where
...
instance KnownNat n => FiniteBits (Int n) where
